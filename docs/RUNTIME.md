...
위치(transform) 트랙은 기준 아이템에 대한 상대 위치다. 기준 아이템이 선
자리는 `Play`로 재생을 **시작할 때마다** 다시 읽는다. `LoadAnimation`으로 미리
불러두고 캐릭터가 걸어간 뒤에 틀어도 지금 선 자리에서 나온다.

다른 트랙이 이미 기준 아이템을 옮기는 중이면 `Arbiter`가 적어둔 원래 자리를
쓴다. 옮겨진 자리를 기준으로 삼으면 둘이 서로를 밀어낸다.

`Play`는 풀어둔 경로도 버린다 (`Applier:ClearCache`). 리스폰했거나 나중에
붙은 파티클을 다시 찾는다. 재생 중에 잡아둔 오브젝트가 지워지면 (`Parent`가
nil) 그 자리에서 다시 푼다.
...
되돌린 뒤에는 적어둔 원래 값을 **버린다.** 남겨두면 그 사이 게임이 속성을
바꿨을 때 다음 애니메이션이 옛 값을 원래 값으로 알고, 끝날 때 게임이 넣은
값을 옛 값으로 덮어쓴다.

마지막 트랙이 끝난 프레임에도 그 트랙이 잡은 값은 쓰인다. 되돌리는 건 다음
Flush라서, 서비스는 재생 중인 트랙이 없어도 **들고 있는 자리가 남아 있으면**
프레임을 한 번 더 돌린다 (`Arbiter:IsIdle`).

섞을 수 없는 값은 페이드아웃 중에도 비중이 0에 닿을 때까지 들고 있는다.
claim에 "멈추는 중" 표시가 같이 가고, `pick`이 그런 claim은 비중이 0보다
크기만 하면 온전한 것으로 친다.
